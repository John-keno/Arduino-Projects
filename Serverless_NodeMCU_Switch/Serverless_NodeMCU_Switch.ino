...
void setup() {
  ...
  if (!SPIFFS.begin()) {
    Serial.println("Error mounting SPIFFS MEMORY");
  }
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("<style>html { font-family: Helvetica; display: inline-block; margin: 0px auto; text-align: center;color: #4CAF50;}");
            ...
            if (output2State == "off") {
              ...
              client.println("<style>body {background: #000000 url(\"/switchedOFF.png\")no-repeat fixed bottom;}</style>");
            } else {
              client.println("<p><a href=\"/2/off\"><button class=\"button button2\">OFF</button></a></p>");
              client.println("<style>body { background: #000000 url(\"/switchedON.png\")no-repeat fixed bottom;}</style>");
            }
